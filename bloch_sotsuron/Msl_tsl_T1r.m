...
for i = 1:size(T1r,2)
  ...
  %Newton's methods
  %-------------------------------------------------------------------------------
  df = @(t) -M(2)/T1r(i)*exp(-t/T1r(i))+exp(-al*t)*( al*M(2)*cos(be*t)+be*M(2)*sin(be*t)+A*al/be*sin(be*t)-A*cos(be*t) );
  ddf = @(t) M(2)/T1r(i)^2*exp(-t/T1r(i))+exp(-al*t)*( -al^2*M(2)*cos(be*t)-2*al*be*M(2)*sin(be*t)-A*al^2/be*sin(be*t)...
